...
export function useCopilotToolCallRenderer(options?: Options) {
  ...
  useRenderToolCall({
    ...
    description: "Transfer control to an agent",
    parameters: [
      {
        name: "agent_name",
        type: "string",
    ...
    render: ({ args, status, result }) => {
      if (status === "inProgress") {
        return (
          <div className="copilotKitMessage toolCallMessage animate-pulse">
            正在召唤：{args.agent_name}...
          </div>
        );
      }
      ...
      if (status === "complete" && result) {
        return (
          <div className="copilotKitMessage toolCallMessage">
            切换至: {args.agent_name}
          </div>
        );
      }
